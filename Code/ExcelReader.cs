			...
			try
			{
				...
				if (this.Headers ==false)
				{
					if (_strSheetRange.IndexOf(":")>0)
					{
                        ...
                        string FirstCol1 = _strSheetRange.Substring(0, _strSheetRange.IndexOf(":")); 
                        string _pattern = @"(?<col>[A-Z]+)\d+$";
                        Match parsed = Regex.Match(FirstCol1, _pattern);
                        string FirstCol = parsed.Groups["col"].Value.ToString();
